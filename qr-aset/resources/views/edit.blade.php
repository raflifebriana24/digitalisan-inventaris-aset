...
<div style="margin-bottom: 2rem;">
    <h2 style="font-size: 1.5rem; font-weight: 600; color: var(--text-primary); margin-bottom: 0.5rem;">
        {{ auth()->user()->role === 'operator' ? 'Update Lokasi Aset' : 'Edit Aset' }}
    </h2>
    <p style="color: var(--text-secondary); font-size: 0.9375rem;">Perbarui informasi aset <strong>{{ $asset->kode_aset }}</strong></p>
</div>
...
<form action="{{ route('assets.update', $asset->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <div style="margin-bottom: 1rem;">
            <label class="form-label" style="font-size: 0.875rem; color: var(--text-muted);">Gambar Saat Ini:</label>
            <div style="padding: 1rem; background: var(--bg-primary); border-radius: var(--radius-md); border: 2px solid var(--border); text-align: center;">
        ...
        <small class="text-muted" style="margin-top: 0.5rem; text-align: center;">Format yang didukung: JPEG, PNG, JPG, WEBP (Maksimal 2MB). {{ $asset->image_path ? 'Upload gambar baru untuk mengganti yang lama.' : '' }}</small>
    </div>
    @endif
    
    <div class="form-group">
        <label for="kode_aset" class="form-label">Kode Aset *</label>
        <input type="text" class="form-control" id="kode_aset" name="kode_aset" value="{{ old('kode_aset', $asset->kode_aset) }}" required {{ auth()->user()->role === 'operator' ? 'readonly style=background-color:var(--bg-tertiary);' : '' }}>
        @if(auth()->user()->role === 'admin')
        <small class="text-muted">⚠️ Mengubah kode aset akan menghasilkan QR Code baru</small>
        @endif
    </div>

    <div class="form-group">
        <label for="nama_aset" class="form-label">Nama Aset *</label>
        <input type="text" class="form-control" id="nama_aset" name="nama_aset" value="{{ old('nama_aset', $asset->nama_aset) }}" required {{ auth()->user()->role === 'operator' ? 'readonly style=background-color:var(--bg-tertiary);' : '' }}>
    </div>

    <div class="form-group">
        <label for="kategori" class="form-label">Kategori *</label>
        @if(auth()->user()->role === 'operator')
            <input type="text" class="form-control" value="{{ $asset->kategori }}" readonly style="background-color: var(--bg-tertiary);">
            <input type="hidden" name="kategori" value="{{ $asset->kategori }}">
        @else
        <select class="form-select" id="kategori" name="kategori" required>
            <option value="">-- Pilih Kategori --</option>
            <option value="Elektronik" {{ old('kategori', $asset->kategori) == 'Elektronik' ? 'selected' : '' }}>Elektronik</option>
            <option value="Furniture" {{ old('kategori', $asset->kategori) == 'Furniture' ? 'selected' : '' }}>Furniture</option>
            ...
            <option value="Lain-lain" {{ old('kategori', $asset->kategori) == 'Lain-lain' ? 'selected' : '' }}>Lain-lain</option>
        </select>
        @endif
    </div>
    ...
    <div class="form-group">
        ...
        @if(auth()->user()->role === 'operator')
        <small class="text-muted" style="color: var(--primary) !important;">Silakan update lokasi aset jika berpindah</small>
        @endif
    </div>
    ...
    <div class="form-group">
        ...
        <input type="number" class="form-control" id="tahun_perolehan" name="tahun_perolehan" value="{{ old('tahun_perolehan', $asset->tahun_perolehan ?? date('Y')) }}" min="1900" max="{{ date('Y') }}" placeholder="Contoh: 2024">
        <small class="text-muted">Tahun saat aset diperoleh atau dibeli</small>
    </div>

    <div class="form-group">
        <label for="deskripsi" class="form-label">Deskripsi</label>
        <textarea class="form-control" id="deskripsi" name="deskripsi" rows="4">{{ old('deskripsi', $asset->deskripsi) }}</textarea>
    </div>
    ...
    <div class="form-group" id="estimatedValueGroup">
        ...
        <div style="position: relative;">
            ...
            <input type="text" class="form-control" id="estimated_value" name="estimated_value" value="{{ old('estimated_value', $asset->estimated_value ? number_format($asset->estimated_value, 0, ',', '.') : '') }}" placeholder="0" style="padding-left: 3rem;" oninput="formatCurrency(this)">
        </div>
    ...
    <div class="form-group">
        ...
        <div style="padding: 1.5rem; background: var(--bg-primary); border-radius: var(--radius-md); border: 2px dashed var(--border); text-align: center;">
            <div class="qr-code" style="display: inline-block;">
                <img src="{{ asset('storage/'.$asset->qr_path) }}" width="150" height="150" alt="QR Code">
            </div>
            <p style="margin-top: 1rem; color: var(--text-muted); font-size: 0.875rem;">
                QR Code untuk: <strong>{{ $asset->kode_aset }}</strong>
            </p>
        </div>
    </div>
    @endif
